...
import {
  marshallComparisonFields,
  marshallSortFields,
  marshallTime
} from "../common/utils/marshall";
import { ObjectID } from "mongodb";
import makeDebug from "debug";

const debug = makeDebug("emiGrup:eCore:query:hooks");
...
export function populateObject(options) {
  return function(hook) {
    ...
    let service =
      _.get(data, options.serviceField) || _.get(query, options.serviceField);
    if (typeof service === "string") {
      const message = `Cannot find the service for ${
        options.serviceField
      } = ${service} to dynamically populate.`;
      ...
      if (options.throwOnNotFound)
        throw new Error(
          `No ${options.serviceField} given to dynamically populate.`
        );
      else return Promise.resolve(hook);
    }
    // Then the object ID
    let id =
      _.get(data, options.idField) ||
      _.get(query, options.idField) ||
      _.get(hook, "id");
    if (!id) {
      if (options.throwOnNotFound)
        throw new Error(
          `Cannot find the ${options.idField} to dynamically populate.`
        );
      else return Promise.resolve(hook);
    }
    // Then the perspective if any
    let perspective =
      _.get(data, options.perspectiveField) ||
      _.get(query, options.perspectiveField);
    ...
    if (typeof id === "string" || ObjectID.isValid(id)) {
      ...
      if (perspective)
        args = Object.assign(args, { query: { $select: [perspective] } });
      return service.get(id.toString(), args).then(object => {
        if (!object) {
          if (options.throwOnNotFound)
            throw new Error(
              `Cannot find object with id ${id} to dynamically populate.`
            );
          else return hook;
        }
...
export function populateObjects(options) {
  return function(hook) {
    ...
    if (!id) {
      ...
      return service
        .find({ query: {}, paginate: false, user: hook.params.user })
        .then(objects => {
      ...
      if (typeof id === "string" || ObjectID.isValid(id)) {
        return service
          .get(id.toString(), { user: hook.params.user })
          .then(object => {
            if (!object) {
              if (options.throwOnNotFound)
                throw new Error(
