...
import { objectifyIDs, toObjectIDs } from "../common/utils/mongoDb";
import { marshallTime, unmarshallTime } from "../common/utils/marshall";
import {
  discard,
  disallow,
  getItems,
...
const debug = makeDebug("emiGrup:eCore:model:hooks");
...
export function processPerspectives(hook) {
  ...
  service.options.perspectives.forEach(perspective => {
    ...
    if (!_.isNil(query) && !_.isNil(query.$select)) {
      ...
      let selectedFields =
        typeof query.$select === "string" ? [query.$select] : query.$select;
    ...
    items.forEach(item => {
      rules.forEach(rule => {
        ...
        if (!_.isNil(source)) {
          ...
          throw new Error(
            "Cannot find valid input value for property " + rule.target
          );
        }
      });
    });
...
export async function populatePreviousObject(hook) {
  if (hook.type !== "before") {
    throw new Error(
      `The 'populatePreviousObject' hook should only be used as a 'before' hook.`
    );
  }
...
export function setExpireAfter(delayInSeconds) {
  return function(hook) {
    if (hook.type !== "before") {
      throw new Error(
        `The 'setExpireAfter' hook should only be used as a 'before' hook.`
      );
    }
    // Retrieve the items from the hook
    let items = getItems(hook);
