...
import { objectifyIDs } from "../common/utils/mongoDb";
import {
  hasServiceAbilities,
...
const debug = makeDebug("emiGrup:eCore:authorisations:hooks");

export function populateSubjects(hook) {
  if (hook.type !== "before") {
    throw new Error(
      `The 'populateSubjects' hook should only be used as a 'before' hook.`
    );
  }
...
export function unpopulateSubjects(hook) {
  if (hook.type !== "after") {
    throw new Error(
      `The 'unpopulateSubjects' hook should only be used as a 'after' hook.`
    );
  }

  return unpopulateObjects({
    serviceField: "subjectsService",
    idField: "subjects"
  })(hook);
}
...
export function populateResource(hook) {
  if (hook.type !== "before") {
    throw new Error(
      `The 'populateResource' hook should only be used as a 'before' hook.`
    );
  }
...
export function unpopulateResource(hook) {
  if (hook.type !== "after") {
    throw new Error(
      `The 'unpopulateResource' hook should only be used as a 'after' hook.`
    );
  }

  return unpopulateObject({
    serviceField: "resourcesService",
    idField: "resource"
  })(hook);
}
...
export function preventEscalation(hook) {
  if (hook.type !== "before") {
    throw new Error(
      `The 'preventEscalation' hook should only be used as a 'before' hook.`
    );
  }
  ...
  debug(
    "Escalation check " +
      (checkEscalation ? "enabled" : "disabled") +
      " for provider"
  );
  ...
  if (checkEscalation) {
    ...
    if (_.isUndefined(role)) {
      ...
      throw new Forbidden(
        `You are not allowed to change authorisation on resource`
      );
    }
    ...
    const subjects = params.subjects.filter(subject => {
      ...
      const subjectResource = _.find(
        subjectScope,
        resource =>
          resource._id &&
          resource._id.toString() === params.resource._id.toString()
      );
      const subjectPermissions = subjectResource
        ? subjectResource.permissions
        : undefined;
      const subjectRole = subjectPermissions
        ? Roles[subjectPermissions]
        : undefined;
    ...
    if (subjects.length < params.subjects.length) {
      debug(
        `${params.subjects.length -
          subjects.length} subjects with higher permissions level found for scope ${scopeName}`
      );
      throw new Forbidden(
        `You are not allowed to change authorisation on subject(s)`
      );
    }
    ...
    if (!_.isUndefined(authorisationRole)) {
      if (authorisationRole > role) {
        debug(
          "Cannot escalate with higher permissions level for scope " + scopeName
        );
        throw new Forbidden(
          `You are not allowed to change authorisation on resource`
        );
...
export function authorise(hook) {
  if (hook.type !== "before") {
    throw new Error(
      `The 'authorise' hook should only be used as a 'before' hook.`
    );
  }
  ...
  debug(
    "Access check " +
      (checkAuthorisation ? "enabled" : "disabled") +
      " for provider"
  );
  ...
  if (checkAuthorisation) {
    ...
    if (!hasServiceAbilities(abilities, hook.service)) {
      ...
      throw new Forbidden(
        `You are not allowed to access service ${hook.service.getPath()}`
      );
    }
    ...
    if (!hook.id) {
      ...
      if (operation === "create") {
        ...
        if (
          ...
          throw new Forbidden(
            `You are not allowed to perform ${operation} operation on ${resourceType}`
          );
        }
      } else {
        ...
        const dbQuery = objectifyIDs(
          getQueryForAbilities(abilities, operation, resourceType)
        );
        if (dbQuery) {
      ...
      return hook.service
        .get(hook.id, Object.assign({ checkAuthorisation: false }, hook.params))
        .then(resource => {
          debug("Target resource is", resource);
