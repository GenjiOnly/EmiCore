...
export const RoleNames = ["member", "manager", "owner"];
...
export function hasResourceAbilities(
  abilities,
  operation,
  resourceType,
  context,
  ...
  if (context)
    object.context =
      typeof context === "object" ? context._id.toString() : context;
...
function buildSubjectsQueryForResource(resourceScope, resourceId, role) {
  let query = { [resourceScope + "._id"]: resourceId };
  if (role) {
    Object.assign(query, { [resourceScope + ".permissions"]: RoleNames[role] });
  }
  return query;
}

export function findSubjectsForResource(
  subjectService,
  resourceScope,
  resourceId,
  role
...
export function countSubjectsForResource(
  subjectService,
  resourceScope,
  resourceId,
  role
