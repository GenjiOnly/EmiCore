...
import OAuth2Verifier from "./common/utils/verifier";
import PasswordValidator from "password-validator";
import { ObjectID } from "mongodb";
import { Database } from "./db";

const debug = makeDebug("emiGrup:eCore:application");
const debugLimiter = makeDebug("emiGrup:eCore:application:limiter");
...
function auth() {
  ...
  if (config.passwordPolicy) {
    ...
    app.getPasswordPolicy = function() {
      ...
      const verifier = new local.Verifier(
        app,
        _.merge(
          { usernameField: "email", passwordField: "password" },
          _.pick(config, ["service"]),
          config.local
        )
      );
...
export function configureService(name, service, servicesPath) {
  ...
  try {
    ...
    debug(
      "No " + name + " service channels configured on path " + servicesPath
    );
    if (error.code !== "MODULE_NOT_FOUND") {
      // Log error in this case as this might be linked to a syntax error in required file
      debug(error);
...
export function createProxyService(options) {
  ...
  return {
    ...
    async get(id, params) {
      return proxyResult(
        await targetService.get(proxyId(id), proxyParams(params))
      );
    },
    async create(data, params) {
      return proxyResult(
        await targetService.create(proxyData(data), proxyParams(params))
      );
    },
    ...
    async remove(id, params) {
      return proxyResult(
        await targetService.remove(proxyId(id), proxyParams(params))
      );
    }
  };
}
...
export function createService(name, app, options = {}) {
  ...
  try {
    if (serviceOptions.modelsPath) {
      const configureModel = require(path.join(
        serviceOptions.modelsPath,
        fileName + ".model." + app.db.adapter
      ));
    ...
    debug(
      "No " +
        fileName +
        " service model configured on path " +
        options.modelsPath
    );
    if (error.code !== "MODULE_NOT_FOUND") {
      // Log error in this case as this might be linked to a syntax error in required file
  ...
  if (dbService) {
    ...
    // service.options içerisinde bulunan id değeri atanır
    ...
    service = require(path.join(
      serviceOptions.servicesPath,
      fileName,
      fileName + ".service"
    ));
    ...
    if (typeof service === "function") {
      ...
      if (typeof service.default === "function")
        service = service.default(name, app, Object.assign({}, serviceOptions));
      else service = service.default;
    }
  ...
  if (serviceOptions.context) {
    contextId =
      typeof serviceOptions.context === "object"
  ...
  if (dbService && serviceOptions.servicesPath) {
    try {
      let serviceMixin = require(path.join(
        serviceOptions.servicesPath,
        fileName,
        fileName + ".service"
      ));
      ...
      if (typeof serviceMixin === "function") {
        ...
        if (typeof serviceMixin.default === "function")
          serviceMixin = serviceMixin.default(
            fileName,
            app,
            Object.assign({}, serviceOptions)
          );
        else serviceMixin = serviceMixin.default;
      }
      ...
      debug(
        "No " +
          fileName +
          " service mixin configured on path " +
          serviceOptions.servicesPath
      );
      if (error.code !== "MODULE_NOT_FOUND") {
        // Log error in this case as this might be linked to a syntax error in required file
...
function setupLogger(logsConfig) {
  ...
  const transports = logsConfig
    ? Object.keys(logsConfig).map(key => {
        const options = logsConfig[key];
        // Setup default log level if not defined
        if (!options.level) {
          options.level =
            process.env.NODE_ENV === "development" ? "debug" : "info";
        }
        return new winston.transports[key](options);
      })
    : [];
...
function tooManyRequests(socket, message, key) {
  ...
  socket.Batolyet("rate-limit", error);
...
function setupSockets(app) {
  ...
  return io => {
    ...
    io.on("connection", socket => {
      nbConnections++;
      debug(
        `New socket connection on server with pid ${process.pid}`,
        socket.id,
      ...
      socket.on("disconnect", () => {
        nbConnections--;
        debug(
          `Socket disconnection on server with pid ${process.pid}`,
          socket.id,
          socket.conn.remoteAddress,
          nbConnections
        );
        if (maxIpConnections > 0) {
          const nbIpConnections =
            _.get(connections, socket.conn.remoteAddress) - 1;
          debug(
            "Total number of connections for",
            socket.id,
            socket.conn.remoteAddress,
            nbIpConnections
          );
      ...
      if (maxConnections > 0) {
        if (nbConnections > maxConnections) {
          tooManyRequests(
            socket,
            "Too many concurrent connections (rate limiting)",
            "RATE_LIMITING_CONCURRENCY"
          );
          return;
        }
      }
        ...
        socket.use((packet, next) => {
          if (packet.length > 0) {
            ...
            if (pathAndMethod.length > 0) {
              ...
              debugLimiter(
                socket.socketLimiter.getTokensRemaining() +
                  " remaining API token for socket",
                socket.id,
                socket.conn.remoteAddress
              );
      ...
      if (authLimiter && authLimiter.websocket) {
        ...
        socket.on("authenticate", data => {
          ...
          if (data.strategy === "local") {
            debugLimiter(
              socket.authSocketLimiter.getTokensRemaining() +
                " remaining authentication token for socket",
              socket.id,
              socket.conn.remoteAddress
            );
...
export function emiGrup() {
  ...
  // Then setup logger
  app.logger = setupLogger(app.get("logs"));
  ...
  app.getService = function(path, context) {
    ...
    if (context && typeof context === "string") {
      ...
      if (ObjectID.isValid(context))
        return app.service(
          app.get("apiPath") + "/" + context.toString() + "/" + path
        );
      else
        return app.service(
          app.get("apiPath") + "/" + context._id.toString() + "/" + path
        );
    } else {
  ...
  app.configure(rest());
  app.configure(
    socketio({ path: app.get("apiPath") + "ws" }, setupSockets(app))
  );
  app.configure(auth);
